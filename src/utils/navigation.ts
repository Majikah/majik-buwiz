 ...
 */
const waitForPathChange = (expectedPath: string, timeout = 20000): Promise<void> => {
...
export async function preloadedNavigation(
  ...
  const navigateWithPreloader = async (): Promise<void> => {
    try {
      // SPA navigation
      navigate(path)
  ...
  toast.promise(navigateWithPreloader(), {
    ...
    error: (error: any) => options?.errorMessage ?? `Navigation failed: ${error.message}`
  })
}
