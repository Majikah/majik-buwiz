...
// Enhanced version with proper TypeScript types:
export const prepareDownloadAnchor = (
  fileType: string,
  filename: string,
): { trigger: (blob: Blob) => Promise<void> } => {
  ...
  return {
    trigger: async (blob: Blob) => {
      // ✅ CRITICAL: Force download by changing MIME type to octet-stream
      const forcedBlob = new Blob([blob], {
        type: "application/octet-stream",
      });

      // Method 1: Share API (best for mobile PWA)
      if (isPWA && navigator.share) {
        try {
          // ✅ FIXED: Use forcedBlob instead of original blob
          const file = new File([forcedBlob], fullFilename, {
            type: "application/octet-stream", // ← Prevents iOS from opening inline
          });

          if (navigator.canShare && navigator.canShare({ files: [file] })) {
            await navigator.share({
              files: [file],
              title: "Majik Buwiz Backup",
              text: "Your account backup key",
            });
            return;
          }
        } catch (err) {
      ...
      // Method 2: File System Access API (desktop PWA only)
      if ("showSaveFilePicker" in window) {
        try {
          // ✅ Fixed: Added angle brackets < > around type parameters
          ...
          const fileConfig = fileTypes[fileType] || {
            ...
            accept: {
              [blob.type || "application/octet-stream"]: [`.${fileType}`],
            },
          };
      ...
      // Method 3: Traditional download (fallback)
      // ✅ Use forcedBlob to prevent inline display
 ...
 * @param {"DEFAULT" | "LETTERS-DIGITS" | "CASED-DIGITS" | "CASED-DIGITS-SYMBOLS"} [requirements="DEFAULT"]
