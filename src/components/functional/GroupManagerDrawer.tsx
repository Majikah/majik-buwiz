...
import { MajikBuwizClient } from "@/SDK/majik-buwiz-client/src/index";
  ...
  const handleToggleMember = async (
    ...
    try {
      if (memberIds.has(contact.id)) {
        await majik.removeContactFromGroup(group.id, contact.id);
        toast.success(`Removed from ${group.meta.name}`, {
          id: `grp-rm-${contact.id}`,
        });
      } else {
        await majik.addContactToGroup(group.id, contact.id);
  ...
  const handleSaveSettings = async (): Promise<void> => {
    ...
    try {
      ...
      if (newColor !== group?.meta?.color) {
        await majik.updateGroupMeta(group.id, { color: newColor });
      }

      if (!isSystem && trimmedName !== group.meta.name) {
        await majik.updateGroupMeta(group.id, { name: trimmedName });
      }
      // Always update description (it carries the color)
      if (editDescription !== group.meta.description) {
        await majik.updateGroupMeta(group.id, { description: editDescription });
      }
