...
export const AppSettingsModal: React.FC<AppSettingsModalProps> = React.memo(
  ({ open, onOpenChange, majik }): JSX.Element => {
    ...
    const handleInvoiceSettingsChange = useCallback(
      async (invoiceDefaults: InvoiceDefaults) => {
        try {
          await majik.stateManager.setInvoiceDefaults(invoiceDefaults);
        } catch (err) {
          console.error("AppSettingsModal: Invoice Defaults save failed", err);
          toast.error("Failed to save setting.");
        }
      },
      [prefs, majik],
