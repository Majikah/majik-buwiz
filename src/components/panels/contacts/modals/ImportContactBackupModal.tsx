...
import type { MajikInvoiceContactGroup } from "@/SDK/majik-buwiz-client/src/core/party/majik-invoice-contact-group";
...
function resolveContactName(contact: MajikInvoiceContact): string {
  const meta = contact.meta;
  if (meta?.legalName?.trim()) return meta.legalName.trim();
  if (meta?.label?.trim()) return meta.label.trim();
  return `${contact.fingerprint.slice(0, 20)}…`;
}

function resolveContactSub(contact: MajikInvoiceContact): string | null {
  const meta = contact.meta;
  const name = resolveContactName(contact);
  if (meta?.tradeName?.trim() && meta.tradeName.trim() !== name)
    return meta.tradeName.trim();
...
const ContactRow: React.FC<ContactRowProps> = memo(
  ({ contact, checked, isDuplicate, onToggle }) => {
    const name = resolveContactName(contact);
    const sub = resolveContactSub(contact);
    const tin = contact.meta?.tin;

    const publicKeyDisplay = useMemo(() => {
      const raw = contact.edPublicKeyBase64 ?? contact.mlKey ?? "";
...
export const ImportContactBackupModal: React.FC<ImportContactBackupModalProps> =
  memo(({ open, onOpenChange, majik, snapshot, onSuccess }) => {
    ...
    const { uniqueContacts, duplicateContacts } = useMemo(() => {
      const unique: MajikInvoiceContact[] = [];
      const duplicates: MajikInvoiceContact[] = [];
    ...
    const handleSelectContactGroup = useCallback(
      (group: MajikInvoiceContact[]) => {
        setSelectedContactIds((prev) => {
          const next = new Set(prev);
    ...
    const handleDeselectContactGroup = useCallback(
      (group: MajikInvoiceContact[]) => {
        setSelectedContactIds((prev) => {
          const next = new Set(prev);
