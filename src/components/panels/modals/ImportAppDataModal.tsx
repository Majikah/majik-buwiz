...
interface SectionDef {
  key: "invoices" | "contacts" | "groups" | "invoiceDefaults" | "preferences";
  label: string;
...
const SECTIONS: SectionDef[] = [
  {
    ...
    sub: (s) =>
      s.invoices.length === 0
        ? "No invoices in this backup"
        : `${s.invoices.length} invoice${s.invoices.length !== 1 ? "s" : ""} will be restored`,
    count: (s) => (s.invoices.length > 0 ? s.invoices.length : null),
    available: (s) => s.invoices.length > 0,
  },
  {
    key: "contacts",
    label: "Contacts",
  ...
  },
  {
    ...
    sub: (s) =>
      s.invoiceDefaults
        ? "Invoice defaults and numbering will be restored"
        : "No invoice settings in this backup",
    count: () => null,
    available: (s) => !!s.invoiceDefaults,
  },
  {
    key: "preferences",
...
export const ImportAppDataModal: React.FC<ImportAppDataModalProps> = memo(
  ({ open, onOpenChange, majik, snapshot, onSuccess }) => {
    ...
    const duplicateCounts = useMemo(() => {
      if (!snapshot) return { contacts: 0, invoices: 0 };
      return {
        contacts: snapshot.contacts.filter((c) => majik.getContactByID(c.id))
          .length,
        invoices: snapshot.invoices.filter((inv) => majik.hasInvoice(inv.id))
          .length,
      };
    }, [snapshot, majik]);
    ...
    const showOverwriteWarning =
      (enabled["contacts"] &&
        ...
        overwriteContacts) ||
      (enabled["invoices"] && duplicateCounts.invoices > 0);
    ...
    const handleConfirm = useCallback(() => {
      ...
      const run = async (): Promise<string> => {
        ...
        if (result.invoices > 0)
          parts.push(
            `${result.invoices} invoice${result.invoices !== 1 ? "s" : ""}`,
          );
              ...
              <SummaryBar>
                {snapshot.invoices.length > 0 && (
                  <>
                    <SummaryChip>
                      <ReceiptIcon size={11} />
                      <span>{snapshot.invoices.length}</span> invoices
                    </SummaryChip>
                    <SummaryDot />
                  </>
                )}
              ...
              {showOverwriteWarning && (
                <WarningRow>
                  ...
                  <div>
                    {enabled["invoices"] && duplicateCounts.invoices > 0 && (
                      <>
                        <strong>{duplicateCounts.invoices}</strong> existing
                        invoice
                        {duplicateCounts.invoices !== 1 ? "s" : ""} will be
                        overwritten.{" "}
                      </>
                    )}
                    {enabled["contacts"] &&
