...
let dbPromise: Promise<IDBPDatabase<ZWorldAutosaveSchema>> | null = null

export async function initDB(): Promise<IDBPDatabase<ZWorldAutosaveSchema>> {
  if (!dbPromise) {
...
export async function idbSaveBlob(id: string, data: Blob): Promise<void> {
